...
export default function useCartSelection() {
    ...
    const addItemToSelection = React.useCallback((itemId: string) => {
        setValue((prev) => [...prev, itemId])
    }, [setValue])
    const removeItemFromSelection = React.useCallback((itemId: string) => {
        setValue((prev) => prev.filter((id) => id !== itemId))
    }, [setValue])
    const clearSelection = React.useCallback(() => {
        removeValue()
    }, [removeValue])
    const setSelection = React.useCallback((itemIds: string[]) => {
        setValue(itemIds)
    }, [setValue])
    return {
        value,
