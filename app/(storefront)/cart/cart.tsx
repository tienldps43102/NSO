...
export default function Cart({ cartItems: initialCartItems }: CartProps) {
  ...
  const { data: cartItems } = useQuery(
    $orpcQuery!.cartRoutes.getMyCartItems.queryOptions({
  ...
  const { mutate: removeCartItem } = useMutation(
    $orpcQuery!.cartRoutes.removeCartItem.mutationOptions({
      onSuccess: (data) => {
  ...
  const { mutate: clearMyCart } = useMutation(
    $orpcQuery!.cartRoutes.clearMyCart.mutationOptions({
      onSuccess: (data) => {
        queryClient.invalidateQueries($orpcQuery!.cartRoutes.getMyCartItems.queryOptions());
  ...
  const { mutate: updateCartItem, isPending: isUpdatingCartItem } = useMutation(
    $orpcQuery!.cartRoutes.updateCartItem.mutationOptions({
      onSuccess: (data) => {
        queryClient.invalidateQueries($orpcQuery!.cartRoutes.getMyCartItems.queryOptions());
            ...
            <div className="bg-card/60 backdrop-blur-sm border border-border/40 rounded-xl p-5 lg:p-6 lg:sticky lg:top-24">
              ...
              <div className="space-y-3 border-t border-border/40 pt-5">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-foreground">{formatPrice(selectedItemsTotalPrice)}</span>
                </div>
